/**
 * FDB-DRV-001/002 — GCP Firestore Provider Adapter
 *
 * In-memory simulation honoring Firestore constraints:
 * - 1MB doc limit, 500-item batch, no aggregation pipeline
 */
...
export class FirestoreAdapter implements IDatastoreRepository {
  readonly domain: EntityDomain;
  private store = new Map<string, Record<string, unknown>>();
  private idempotencyKeys = new Map<string, string>();
  private caps = PROVIDER_CAPABILITIES.firestore;
  ...
  async create(doc: Record<string, unknown>, opts?: WriteOptions): Promise<WriteResult> {
    if (JSON.stringify(doc).length > this.caps.maxDocSizeBytes)
      throw new Error('Document exceeds Firestore 1MB limit');
    // Idempotency check
  ...
  async createMany(docs: Record<string, unknown>[], opts?: WriteOptions): Promise<BatchWriteResult> {
    if (docs.length > this.caps.maxBatchSize) throw new Error(`Batch size exceeds Firestore limit of ${this.caps.maxBatchSize}`);
  ...
  getCapabilities(): ProviderCapabilities { return PROVIDER_CAPABILITIES.firestore; }
...
export class FirestoreAdapterFactory implements RepositoryFactory {
  create(domain: EntityDomain, _config?: Record<string, unknown>): IDatastoreRepository { return new FirestoreAdapter(domain); }
  capabilities(): ProviderCapabilities { return PROVIDER_CAPABILITIES.firestore; }
}
