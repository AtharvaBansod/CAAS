/**
 * FDB-DRV-001/002 — AWS DynamoDB Provider Adapter
 *
 * In-memory simulation of DynamoDB behavior honoring DynamoDB constraints:
 * - 400KB item limit, 25-item batch limit
 * - No aggregation pipeline, no full-text search
 * - TTL support, conditional writes, atomic counters
 */
...
import { RepositoryFactory } from '../../abstraction';

export class DynamoDBAdapter implements IDatastoreRepository {
  readonly domain: EntityDomain;
  private store = new Map<string, Record<string, unknown>>();
  private idempotencyKeys = new Map<string, string>();
  private caps = PROVIDER_CAPABILITIES.dynamodb;
  ...
  async create(doc: Record<string, unknown>, opts?: WriteOptions): Promise<WriteResult> {
    ...
    if (serialized.length > this.caps.maxDocSizeBytes) {
      throw new Error(`Item exceeds DynamoDB 400KB limit (${serialized.length} bytes)`);
    }
    // Idempotency check
  ...
  async createMany(docs: Record<string, unknown>[], opts?: WriteOptions): Promise<BatchWriteResult> {
    if (docs.length > this.caps.maxBatchSize) {
      throw new Error(`Batch size ${docs.length} exceeds DynamoDB limit of ${this.caps.maxBatchSize}`);
    }
    const succeeded: WriteResult[] = [];
    const failed: Array<{ id?: string; error: string }> = [];
    for (const doc of docs) {
      try {
        succeeded.push(await this.create(doc, opts));
      } catch (err: any) {
  ...
  async updateMany(filter: FilterSpec, patch: Record<string, unknown>, opts?: WriteOptions): Promise<UpdateResult> {
    ...
    for (const [id, doc] of this.store.entries()) {
      ...
      if (this.matchesFilter(doc, filter)) {
        matched++;
        this.store.set(id, { ...doc, ...patch, _version: ((doc._version as number) ?? 0) + 1 });
        modified++;
      }
    }
  ...
  async delete(id: string, opts?: WriteOptions): Promise<boolean> {
    const doc = this.store.get(id);
    if (!doc) return false;
    if (opts?.tenantId && doc._tenantId !== opts.tenantId) return false;
  ...
  getCapabilities(): ProviderCapabilities {
    return PROVIDER_CAPABILITIES.dynamodb;
  }
...
export class DynamoDBAdapterFactory implements RepositoryFactory {
  create(domain: EntityDomain, _config?: Record<string, unknown>): IDatastoreRepository {
    return new DynamoDBAdapter(domain);
  }
